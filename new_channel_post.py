...
def delete_post_by_user_id(user_id, message_id, file_name):
    ...
    if user_id in scheduled_channel_posts:
        scheduled_channel_posts[user_id] = [
            post
            for post in scheduled_channel_posts[user_id]
...
async def set_time(update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if post_time < datetime.now():
            await update.message.reply_text(
                "Дата и время должны быть позже текущего момента."
            )
            return
        ...
        await update.message.reply_text(
            "Ошибка! Проверьте формат даты и времени (YYYY-MM-DD HH:MM)."
        )
...
def forward_post_async(bot, chat_id, text, photo_id, user_id, message_id, user_chat_id):
    ...
    loop.run_until_complete(
        forward_post(bot, chat_id, text, photo_id, user_id, message_id, user_chat_id)
    )
