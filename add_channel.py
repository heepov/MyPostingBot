...
async def add_channel_link(update: Update, context: CallbackContext) -> None:
    ...
    if not channel_link:
        await update.message.reply_text(
            "Неверная ссылка на канал. Пожалуйста, отправьте правильную ссылку или @имя канала."
        )
        return
    ...
    try:
        ...
        if not is_admin_channel:
            await update.message.reply_text(
                f"Бот не добавлен в канал @{channel.username} или не имеет нужных прав."
            )
            return
        ...
        await update.message.reply_text(
            f"Произошла ошибка при добавлении канала {channel_link}. Ошибка: {str(e)}"
        )
...
async def add_chat_link(update: Update, context: CallbackContext) -> None:
    ...
    if not channel_link:
        await update.message.reply_text(
            "Неверная ссылка на ЧАТ. Пожалуйста, отправьте правильную ссылку или @имя канала."
        )
        return
    ...
    try:
        ...
        if not is_admin_channel:
            await update.message.reply_text(
                f"Бот не добавлен в чат @{chat.username} или не имеет нужных прав."
            )
